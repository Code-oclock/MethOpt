...
for name in methods:
    if name in ("SGD", "SGD+Momentum", "SGD+Nesterov"):
        ...
        if name == "SA_lin":
            scheduling = lib.temperature_schedule('linear')
        if name == "SA_exp":
            scheduling = lib.temperature_schedule('exponential')
        if name == "SA_log":
            scheduling = lib.temperature_schedule('logarithmic')
        if name == "SA_const":
            scheduling = lib.temperature_schedule('constant_step')

        _, mse_final, history = lib.simulated_annealing(
            obj_func=lambda w: mean_squared_error(y, X @ w),
            x0=x0.copy(),
            T0=1.0,
            t_sheduling=scheduling,
            n_iter=ERAS,
            bounds=bounds,
            step_size=0.5
        )
